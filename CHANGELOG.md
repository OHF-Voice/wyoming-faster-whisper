...
- Use a Lithuanian fine-tune as the faster-whisper default for `--language lt`
  - Lithuanian is the one language where the generic defaults are not merely weaker but unusable. Measured on FLEURS `lt_lt` test (986 utterances, 17,136 reference words, identical normalisation and scoring for every model): `faster-whisper-base` (the CPU default) **87.84%** WER, `faster-whisper-small` (the GPU default) **65.79%**, `faster-whisper-large-v3` 24.23%, and `paprika-whisper-lt-v3` **12.11%**
  - At 87.84% the default does not produce a worse transcript, it produces no usable transcript at all — and it fails silently: the pipeline stays green, the sentences look like sentences, and Home Assistant answers the wrong thing. Nothing in the logs says the language is the problem
  - The model is a CTranslate2 int8 conversion of [`kristijonas/paprika-whisper-lt-v3`](https://huggingface.co/kristijonas/paprika-whisper-lt-v3) (CC BY 4.0) — a `whisper-large-v3-turbo` fine-tune trained on 3,281 h of the LIEPA-3 corpus by Kristijonas Jakubsonas, who published it openly. The conversion is the only part that is ours; the model is his
  - Published in a single quantization, like the other backends' defaults, so the GPU and arm branches do not apply. It is int8 but ~800M parameters — larger than the `small` the GPU branch would otherwise pick — and runs at 0.23 s per utterance on an RTX PRO 4500 against 0.35 s for `small`, so the accuracy does not cost latency here
  - Scope is deliberately narrow: only the faster-whisper default for one language code. Anything passed with `--model` is untouched, and no other language changes
  - Two caveats worth stating rather than hiding. FLEURS is **read speech**, which the fine-tune's own author names as its stronger ground, so this table understates how the other lithuanian model would compare on spontaneous speech; and 18% of FLEURS `lt` references print numbers as digits the speaker never said, which penalises every model that verbalises them — both the fine-tune and the generic Whisper do, so the comparison stays fair, but the absolute figures are pessimistic for all of them
